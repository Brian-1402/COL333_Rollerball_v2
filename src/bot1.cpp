...
std::pair<U16, double> minimax_node(Board *b, int depth, std::atomic<bool> &search, int ismax)
{
    ...
    if (!search)
        return {0xfffe, 0}; // search tracker

    auto moveset = b->get_legal_moves();
    if (!moveset.size())
    {
        if (b->in_check())
            return {0xffff, -ismax * 1e9}; // checkmate
        else
            return {-1, 0}; // stalemate
    }
    if (depth == maxD)
        return {0xffff, 1.0 * ismax * piece_advantage(b)};
    ...
    for (auto curr : moveset)
    {
        ...
        if (adversary.first != 0xfffe)
        {
            ...
            else if (abs(ismax * adversary.second - ismax * best_pair.second) < 1e-5)
            { // minimise the manhattan dist from the opponent king for game progression.
...
void Engine::find_best_move(const Board &b)
{
    ...
    else
    {
        ...
        while (search)
        {
            ...
            if (best_pair.first != 0xfffe and best_pair.first != 0xffff)
                this->best_move = best_pair.first;
